...
Given that you've determined that the contianer image's distro is either `wolfi` or `chainguard`, you can find a database of all distro packages that have been installed at `/usr/lib/apk/db/installed`. Note previously, the database was at `/lib/apk/db/installed` and was moved during the [usrmerge transition](https://www.chainguard.dev/unchained/wolfi-moves-to-usrmerge-standard). The specification for this "installed database" is documented [here](https://wiki.alpinelinux.org/wiki/Apk_spec#Installed_Database_V2). The fields we're interested in are documented [here](https://wiki.alpinelinux.org/wiki/Apk_spec#APKINDEX_Format).
